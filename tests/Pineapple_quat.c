...
struct logger {
    ...
    double lambda[STEPS];
    double theta[STEPS];
    double lambda_s[STEPS];
    double theta_s[STEPS];
};
...
void write_to_files(struct walker *w, struct planet *p, struct simulation_parameters *sim, struct logger *log);
...
void log_vals(struct walker *w, struct planet *p, struct simulation_parameters *sim, struct logger *log, int i) {
    log->times[i] = sim->t;
    log->lambda[i] = w->lambda;
    log->theta[i] = w->theta;
    log->lambda_s[i] = p->lambda_s;
    log->theta_s[i] = p->theta_s;
};
...
void simulation(struct walker *w, struct planet *p, struct simulation_parameters *sim, struct logger *log) {
...
int main() {
    ...
    clock_t begin = clock();
    simulation(&w, &p, &sim, &log);
    clock_t end = clock();

    double time_spent = (double)(end - begin) / CLOCKS_PER_SEC;
    printf("DONE, Time: %lf\n", time_spent);
    write_to_files(&w, &p, &sim, &log);
    clock_t end2 = clock();
    double time_spent2 = (double)(end2 - end) / CLOCKS_PER_SEC;
    printf("DONE WRITING, Time: %lf\n", time_spent2);
...
void write_to_files(struct walker *w, struct planet *p, struct simulation_parameters *sim, struct logger *log) {
    ...
    for (int i = 0; i < STEPS; i++) {
        ...
        log->times[i], log->theta[i], log->lambda[i], log->theta_s[i], log->lambda_s[i]);
    }
