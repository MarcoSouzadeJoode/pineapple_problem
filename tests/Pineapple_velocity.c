...
int single_run(double v) {
    clock_t begin = clock();
    char filename[64];
    sprintf(filename, "vel/VELTEST_%lf.txt",v);

    int N = 1576800; // iterations -> 5 years
    double dt = 100; // seconds

    double vector[] = {1,0.0,1}; // initial position
