...
void looper(int N, double dt, double omega, double *rsun,
 double *vector, double cos_theta, int dim, const char *filename) {
    ...
    for (int j = 0; j < N; j++) {
        ...
        //fprintf(file, "%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\n", t, vector[0], vector[1],vector[2], rsun[0],rsun[1],rsun[2]);
...
int single_run(double v) {
    clock_t begin = clock();
    char filename[64];
    sprintf(filename, "seasons/VELTEST_%lf.txt",v);

    int N = 15768000 / 5 / 2.0 / 10; // iterations -> 5 years
    double dt = 500; // seconds
    ...
    double cos_theta, R_planet, omega, T;
    ...
    looper(N, dt, omega, rsun, vector, cos_theta, dim, filename);
    clock_t end = clock();
